...
function buildSchedule(entries) {
  ...
  for (const {track, start, end, title} of entries) {
    ...
    if (isValidTime(start) && isValidTime(end) && toMinutes(end) > toMinutes(start)) {
  ...
  for (const track of sortedTracks) {
    ...
    for (const entry of trackMap.get(track)) {
      if (!endTimeMap.has(entry.end)) endTimeMap.set(entry.end, {entry})
    }
    const sortedEndTimes = [...endTimeMap].sort();

    const scheduleMap = new Map();
    for (const time of sortedEndTimes) {
